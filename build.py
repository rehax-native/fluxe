...
if is_win:
  ...
  # os.system('cd build && ninja fluxe-win && cp libfluxe-win.a ../dist/fluxe.lib')

  # Building with something other than VS doesn't work on windows.
  # Skia is built with VS, so the library it generates has VS mangled names.
  # If you try to link against that with code generated by something else than VS, you'll get undefined reference errors
